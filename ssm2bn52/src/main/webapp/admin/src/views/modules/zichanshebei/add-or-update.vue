          ...
          <el-form-item class="input" label="物品名称" prop="wupinmingcheng">
              <el-input v-model="ruleForm.wupinmingcheng" 
                placeholder="物品名称" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="物品类型" prop="wupinleixing">
          <el-select v-model="ruleForm.wupinleixing" placeholder="请选择物品类型">
            <el-option
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `zichanshebei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zichanshebeiCrossAddOrUpdateFlag = false;
                  this.parent.search();
